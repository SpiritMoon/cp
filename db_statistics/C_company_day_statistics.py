...
def stat_company_day_statistics_for_date(conn, cur, day, cid):
    """
    通过日期和客户id统计客户数据
    """
    ...
    # 查询客户的此日期记录在表company_day_statistics是否存在
    cur.execute("SELECT id FROM company_day_statistics WHERE c_id = %d AND day = '%s'" % (cid, day))
    cdsid = cur.fetchone()
    if cdsid is None:
        # 不存在则插入记录
        cur.execute("""INSERT INTO company_day_statistics
                ...
                %d, %d, %d, %d, %d, %d,
                %d)"""
                % ((cid, day) + company_data) )
    else:
        # 存在则更新记录
        cur.execute("""UPDATE company_day_statistics
...
def stat_company_day_statistics(conn, cur, cids, str_yesterday):
    """
    统计客户下的用户上网信息
    先统计昨日的
    然后根据更新时间统计 统计之后有更新的
    ...
    for cid in cids:
        stat_company_day_statistics_for_date(conn, cur, str_yesterday, cid[0])
        
        cur.execute("""
                SELECT
                    customer_day_statistics. DAY
                FROM
                    customer_day_statistics,
                    company_day_statistics
                WHERE
                    customer_day_statistics. DAY = company_day_statistics. DAY
                AND customer_day_statistics.updated_at > company_day_statistics.updated_at
                AND company_day_statistics.c_id = %d
                AND customer_day_statistics.c_id IN
                    (SELECT ID FROM customer WHERE company_id IN
                        (SELECT ID FROM company WHERE p_id = %d OR id = %d)
                    )
                GROUP BY
        ...
        days = cur.fetchall()
        for day in days:
            stat_company_day_statistics_for_date(conn, cur, day[0], cid[0])
        
        
def stat_customer_table(conn, cur, cids):
    """
    统计存放在customer表中的统计数据：平均在线市场，总认证人数，最近30天的上网次数
    """
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), " -- ", "stat customer table..." )
    ...
    for cid in cids:
        ...
        cur.execute("""
                ...
                FROM customer
                WHERE company_id IN (SELECT ID FROM company WHERE p_id = %d OR id = %d)"""
                % (cid[0], cid[0]) )
        ...
        for customer_data in customers_data:
            print(customer_data)
            per_capita_duration += (customer_data[0] * customer_data[1])
            per_num += customer_data[1]
            
            # 切出来最近30天的上网次数数据
            last30_conn_num_stat = customer_data[2].split(',')
...
if __name__ == "__main__":
    ...
    # 统计wifi log
    stat_company_day_statistics(conn, cur, cids, str_yesterday)
    print("\n")

    # 统计总上网人数和平均上网时长 最近30天的用户上网次数
    stat_customer_table(conn, cur, cids)
    print("\n")
